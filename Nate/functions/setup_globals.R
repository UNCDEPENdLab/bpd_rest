...
roiFile <- file.path(basedir, "bb264coordinate_appended_shift_nate_culled.txt")
roiMat <- read.table(roiFile, header=FALSE, col.names=c("x", "y", "z", "roi"))
atlasFile <- paste0(parcellation, "_atlas.csv")
atlas <- read.csv(atlasFile, header = TRUE, stringsAsFactors = FALSE) #eventually would be nice to have a lockstep name here and re-use parcellationName
use.infomap <- 1

preproc_pipeline <- "aroma" #method for data preprocessing. This corresponds to mni_5mm_aroma data

#conn_method <- "aroma_pearson" #Jun2017: aroma preprocessing, pearson correlations
