...
#                          reducemetrics = c("degree",  "eigen.cent", "betweenness.node", "page.rank",  "part.coeff", "gateway.coeff.btw",  "within.module.deg"),
...
# can be sourced from main pipeline ---------------------------------------


trans.file <- paste0(basedir, "/cache/threshnodalmetrics_transformed_20rs_", file_tag, ".RData")
if(file.exists(trans.file)){
  ...
  ###2/23/18: set NAs to be equal to 0. Right now, leverage centrality, both gateway coefficients, and participation coefficients have some NAs. These will flag isolated nodes or nodes with no intermodualr connections and set them to 0
  # allmetrics.nodal.df.trans[is.na(allmetrics.nodal.df.trans)] <- 0 
  # is.nan.data.frame <- function(x)
  #   do.call(cbind, lapply(x, is.nan))
  # 
  # allmetrics.nodal.df.trans[is.nan(allmetrics.nodal.df.trans)] <- 0
  # 
  ...
  allmetrics.nodal.df.trans$eigen.cent <- Winsorize(allmetrics.nodal.df.trans$eigen.cent, minval = quantile(allmetrics.nodal.df.trans$eigen.cent, .001), maxval = quantile(allmetrics.nodal.df.trans$eigen.cent, .999))
  ...
  allmetrics.nodal.df.trans$page.rank <- Winsorize(allmetrics.nodal.df.trans$page.rank, minval = quantile(allmetrics.nodal.df.trans$page.rank, .005), maxval = quantile(allmetrics.nodal.df.trans$page.rank, .995))
  allmetrics.nodal.df.trans$leverage.cent <- Winsorize(allmetrics.nodal.df.trans$leverage.cent, minval = quantile(allmetrics.nodal.df.trans$leverage.cent, .001, na.rm = TRUE), maxval = quantile(allmetrics.nodal.df.trans$leverage.cent, .999, na.rm = TRUE))
  wi <- (1/1+(allmetrics.nodal.df.trans$within.module.deg))
  allmetrics.nodal.df.trans$within.module.deg <- Winsorize(wi, minval = quantile(wi, .005, na.rm = TRUE), maxval = quantile(wi, .995, na.rm = TRUE))
  ...
  part.coeff <- Winsorize(allmetrics.nodal.df.trans$part.coeff, minval = quantile(allmetrics.nodal.df.trans$part.coeff, .01, na.rm = TRUE), maxval = quantile(allmetrics.nodal.df.trans$part.coeff, .999, na.rm = TRUE))
  
  allmetrics.nodal.df.trans$part.coeff <- BoxCox_extract(part.coeff, seq(5,20,.1))
  
  
  ##gateway coefficient
  gateway.btw <- Winsorize(allmetrics.nodal.df.trans$gateway.coeff.btw, minval = quantile(allmetrics.nodal.df.trans$gateway.coeff.btw, .01, na.rm = TRUE), maxval = quantile(allmetrics.nodal.df.trans$gateway.coeff.btw, .999, na.rm = TRUE))
  
  allmetrics.nodal.df.trans$gateway.coeff.btw <- BoxCox_extract(gateway.btw, seq(-3,25,.1))
  
  #based on degree
  gateway.degree <- Winsorize(allmetrics.nodal.df.trans$gateway.coeff.degree, minval = quantile(allmetrics.nodal.df.trans$gateway.coeff.degree, .01, na.rm = TRUE), maxval = quantile(allmetrics.nodal.df.trans$gateway.coeff.degree, .999, na.rm = TRUE))
  ...
  #allmetrics.nodal.df.trans <- nodalmetrics_df
