...
inputs <- specify_inputs(thresh_weighted = "binary", 
                         ...
                         reducemetrics = c("eigen.cent", "betweenness.node", "page.rank",  "part.coeff", "gateway.coeff.btw",  "within.module.deg"),
                         rs_desired_log = logspace(log10(.01), log10(.02), 10)) 
#PEARSON
...
trans.file <- paste0(basedir, "/cache/threshnodalmetrics_transformed_10rs", file_tag, ".RData")
...
allmetrics.nodal.df.trans$eigen.cent <- (allmetrics.nodal.df.trans$eigen.cent)^2
...
allmetrics.nodal.df.trans$within.module.deg <- Winsorize(allmetrics.nodal.df$within.module.deg, minval = quantile(allmetrics.nodal.df$within.module.deg, .001, na.rm = TRUE), maxval = quantile(allmetrics.nodal.df$within.module.deg, .999, na.rm = TRUE))
qplot(allmetrics.nodal.df.trans$betweenness.node)
