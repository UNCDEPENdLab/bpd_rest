...
# Gather self reports and score for SEM ---------------------------------------------
...
#SPECC ID 066DW: missing items 4,6,7,8,11,27,49
...
faout_upps <- fa(dplyr::select(upps.complete, NegUrg, LackPers, PosUrg), nfactors = 1, fm = "ml")
faout_upps$loadings
...
ders.sem <- sem(DERS_mod, data = ders.complete, estimator = "MLR")#,  missing = "FIML") 
semPaths(ders.sem)
summary(ders.sem, fit.measures = TRUE)
modificationindices(ders.sem) %>% dplyr::filter(mi > 2) %>% arrange(desc(mi)) %>% head(20)
rowMeans(cor(dplyr::select(ders.complete, NonAccept, Goals, Impulse, Strategy, Clarity), use = "complete.obs"))
resid(ders.sem, type = "cor")

faout_ders <- fa(dplyr::select(ders.complete, NonAccept, Goals, Impulse, Strategy, Clarity), nfactors = 1, fm = "ml")
faout_ders$loadings
...
faout_panas.negaff <- fa(dplyr::select(panas.negaff.complete, PANAS.2, PANAS.4, PANAS.6, PANAS.7, PANAS.8, PANAS.11, PANAS.13, PANAS.15, PANAS.18, PANAS.20), nfactors = 1, fm = "ml")
faout_panas.negaff$loadings



# BPQ - load scored self reports ------------------------------------------
...
bpq.sem <- sem(BPQ_mod, data = bpq.complete, estimator = "MLR")#,  missing = "FIML") 
semPaths(bpq.sem)
summary(bpq.sem, fit.measures = TRUE)

resid(bpq.sem, type = "cor")

faout_bpq <- fa(dplyr::select(bpq.complete, impulsivity, instability, abandonment, relationships, selfimage, suicide, emptiness, anger), nfactors = 1, fm = "ml")
faout_bpq$loadings
...
table(is.na(selfreports$IIP_PD1))

#examine alphas
a <- alpha(IIP.complete)
a$total$raw_alpha
a <- alpha(BPQ[,7:96])
a$total$raw_alpha
a <- alpha(ders.complete)
a$total$raw_alpha
a <- alpha(upps.complete)
a$total$raw_alpha
