...
brainGraph_GLM <- function(g.list, covars, measure, con.mat, con.type=c('t', 'f'),
  ...
  if (isTRUE(permute)) {
    ...
    if(!length(zeroregions) == 0) {
      null.dist <- randomise(ctype, N, perms, DT.m[!region %in% zeroregions], nC, measure, X, con.mat, alt)
    } else {
      null.dist <- randomise(ctype, N, perms, DT.m, nC, measure, X, con.mat, alt)
    }
...
brainGraph_GLM_fit_t <- function (X, y, XtX, con.mat) 
{
  ...
  suppressMessages(require(RcppBlaze))
  est <- fastLmPure(X, y, type = 2)
  
  #est <- fastLmPure(X, y, method=2) #in original brainGraph script
  b <- est$coefficients
  gamma <- con.mat %*% b
