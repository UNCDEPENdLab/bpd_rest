...
use.yeo <- 1
data.reduce <- "fa" #can be fa or pca
weighted.reduce <- NULL #run fa/pca on weighted graphs?
reduce.method <- "a_priori" #can be set to "all", "a_priori", or "metrics". This specifies how data reduction step is performed across metrics and densities (for binary analyses)
node.file.dir <- file.path(basedir, "Viz_files")
include.weighted <- 1 #0 = no 1 = yes
...
adjmat_extension <- ".txt.gz" #used to ID adjmats to import into pipeline
...
reducemetrics <- c("eigen.cent","degree", "betweenness.node", "page.rank",  "part.coeff", "gateway.coeff.btw", "gateway.coeff.degree", "within.module.deg")#, "closeness", "between.module.deg", "leverage.cent")
...
} else {densities_desired <- seq(.05, .20, .01)}
...
message("Nodal metrics calculated: ", metricstorun.nodal)
message("Global metrics calculated: ", metricstorun.global)
